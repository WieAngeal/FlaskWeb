...
var des3iv = '20170908';//和java中的iv保持一致
var DES3 = {
	//3DES加密，使用PKCS7 padding  
	//key为随机生成的10位时间戳， key长度为24，自己定义,和后台java保持一致即可，现在是每次登陆把时间戳送到后台解密
	encrypt:function(key,input){
	    key="publichealthss"+key;
		...
		if(window.btoa){
			return btoa(des(genKey.key, input, 1, 1, des3iv, 1));
			 ...
			 var b = new Base64();   
			 return b.encode(des(genKey.key, input, 1, 1, des3iv, 1));
		}
	},
