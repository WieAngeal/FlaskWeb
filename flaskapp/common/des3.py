...
DES_SECRET_KEY = "870111201611063710"
...
def des3_encrypt(text):
    # 待加密文本
    # 初始化加密器
    aes = DES3.new(add_to_24(DES_SECRET_KEY), DES3.MODE_ECB)
    #先进行aes加密
    encrypt_aes = aes.encrypt(add_to_24(text))
    #用base64转成字符串形式
    encrypted_text = str(base64.encodebytes(encrypt_aes), encoding='utf-8')  # 执行加密并转码返回bytes
...
def des3_decrypt(text):
    # 密文
    # 初始化加密器
    des = DES3.new(add_to_24(DES_SECRET_KEY), DES3.MODE_ECB)
    #优先逆向解密base64成bytes
    base64_decrypted = base64.decodebytes(text.encode(encoding='utf-8'))
    #执行解密密并转码返回str
    decrypted_text = str(des.decrypt(base64_decrypted),encoding='utf-8').replace('\0','')
